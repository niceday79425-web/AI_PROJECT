...
<head>
  ...
  <style>
    ...
    #progress-bar {{ position:fixed;top:0;left:0;height:3px;width:0%;background:linear-gradient(90deg,#6366f1,#8b5cf6);z-index:9999;transition:width 0.1s linear; }}
    .post-hero {{ background:linear-gradient(135deg,#0f0c29,#302b63,#24243e);color:#fff;padding:4rem 1.5rem 3rem;text-align:center; }}
    .series-badge {{ display:inline-block;background:rgba(99,102,241,0.8);color:#fff;font-size:0.8rem;font-weight:700;padding:0.35rem 1rem;border-radius:999px;margin-bottom:1.2rem; }}
    .post-hero h1 {{ font-size:clamp(1.5rem,4vw,2.4rem);font-weight:900;line-height:1.4;max-width:820px;margin:0 auto 1rem; color:#fff; }}
    .post-content {{ max-width:860px;margin:0 auto;padding:2.5rem 1.5rem; }}
    .post-content h2 {{ font-size:1.4rem;font-weight:700;color:var(--text-primary);margin:2.5rem 0 1rem;border-left:4px solid #6366f1;padding-left:0.75rem; }}
    .post-content h3 {{ font-size:1.15rem;font-weight:700;color:var(--text-primary);margin:1.8rem 0 0.75rem; }}
    .post-content p {{ line-height:2.0;color:var(--text-primary);margin-bottom:1.2rem; font-size:1.05rem; }}
    .post-content ul, .post-content ol {{ padding-left:1.5rem;margin-bottom:1.2rem; }}
    .post-content li {{ line-height:2.0;color:var(--text-primary);margin-bottom:0.5rem; font-size:1.05rem; }}
    .post-content strong {{ color:var(--text-primary); font-weight:700; }}
    .key-point {{ background:rgba(99,102,241,0.1);border-left:4px solid #6366f1;border-radius:0 10px 10px 0;padding:1.2rem 1.5rem;margin:1.5rem 0; color:var(--text-primary); font-size:1.05rem; }}
    .key-point strong {{ color:#a78bfa; }}
    .data-table {{ width:100%;border-collapse:collapse;margin:1.5rem 0;font-size:0.95rem; }}
    .data-table th {{ background:rgba(255,255,255,0.1);color:var(--text-primary);padding:0.75rem 1rem;text-align:left; border-bottom:2px solid var(--border-color); }}
    .data-table td {{ padding:0.7rem 1rem;border-bottom:1px solid var(--border-color);color:var(--text-secondary); background:var(--glass-bg); }}
    .data-table tr:nth-child(even) td {{ background:rgba(255,255,255,0.02); }}
    .nav-links {{ display:flex;gap:1rem;flex-wrap:wrap;margin:2rem 0;padding:1.2rem;background:var(--card-bg);border:1px solid var(--border-color);border-radius:10px; }}
    .nav-links a {{ color:#a78bfa;text-decoration:none;font-weight:600;font-size:0.95rem; }}
    .author-box {{ display:flex;align-items:center;gap:1rem;background:var(--card-bg);border:1px solid var(--border-color);border-radius:12px;padding:1rem 1.2rem;margin-bottom:2rem; }}
    .author-avatar {{ width:48px;height:48px;background:linear-gradient(135deg,#6366f1,#8b5cf6);border-radius:50%;display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#fff;font-weight:700;font-size:1.1rem; }}
    .author-box strong {{ color:var(--text-primary); }}
    .author-box span {{ color:var(--text-secondary); }}
    .disclaimer {{ margin-top:3rem;padding:1.25rem 1.5rem;background:rgba(239,68,68,0.05);border-left:4px solid #ef4444;border-radius:10px;font-size:0.85rem;color:var(--text-secondary);line-height:1.7; }}
    .back-btn {{ display:inline-flex;align-items:center;gap:0.4rem;color:#a78bfa;font-weight:600;text-decoration:none;font-size:0.95rem;margin-bottom:1.5rem; }}
    .series-nav {{ background:rgba(99,102,241,0.05);border-radius:12px;padding:1.25rem 1.5rem;margin-top:3rem; border:1px solid var(--border-color); }}
    .series-nav h3 {{ color:#a78bfa;font-size:1rem;margin-bottom:0.75rem; }}
